...
var MiniFileMgr$6=(function(){
	...
	__class(MiniFileMgr,'laya.qq.mini.MiniFileMgr',null,'MiniFileMgr$6');
...
var MiniImage$6=(function(){
	function MiniImage(){}
	__class(MiniImage,'laya.qq.mini.MiniImage',null,'MiniImage$6');
...
var MiniInput$6=(function(){
	function MiniInput(){}
	__class(MiniInput,'laya.qq.mini.MiniInput',null,'MiniInput$6');
...
var MiniLocalStorage$6=(function(){
	...
	__class(MiniLocalStorage,'laya.qq.mini.MiniLocalStorage',null,'MiniLocalStorage$6');
...
var MiniLocation$6=(function(){
	...
	__class(MiniLocation,'laya.qq.mini.MiniLocation',null,'MiniLocation$6');
...
var MiniVideo$4=(function(){
	...
	__class(MiniVideo,'laya.qq.mini.MiniVideo',null,'MiniVideo$4');
...
var QQMiniAdapter=(function(){
	...
	QQMiniAdapter.init=function(isPosMsg,isSon){
		...
		if(!QQMiniAdapter.isZiYu){
			...
			MiniFileMgr$6.existDir(MiniFileMgr$6.fileNativeDir,Handler.create(QQMiniAdapter,QQMiniAdapter.onMkdirCallBack));
		}
	...
	QQMiniAdapter.onMkdirCallBack=function(errorCode,data){
		if (!errorCode){
			...
			MiniFileMgr$6.fakeObj=MiniFileMgr$6.filesListObj;
		}
	}
...
var MiniAccelerator$6=(function(_super){
	...
	__class(MiniAccelerator,'laya.qq.mini.MiniAccelerator',_super,'MiniAccelerator$6');
...
var MiniLoader$6=(function(_super){
	...
	__class(MiniLoader,'laya.qq.mini.MiniLoader',_super,'MiniLoader$6');
	...
	*/
	__proto.load=function(url,type,cache,group,ignoreCache){
		...
		if ((MiniLoader._fileTypeArr.indexOf(urlType)!=-1)|| type==/*laya.net.Loader.IMAGE*/"image"){
			...
			if(QQMiniAdapter.isZiYu && !MiniFileMgr$6.ziyuFileData[url]){
				url=URL.formatURL(url);
			}
			if(QQMiniAdapter.isZiYu && MiniFileMgr$6.ziyuFileData[url]){
			...
			if (!MiniFileMgr$6.getFileInfo(URL.formatURL(url))){
				if (MiniFileMgr$6.isLocalNativeFile(url)){
					if (QQMiniAdapter.subNativeFiles && QQMiniAdapter.subNativeheads.length==0){
						for (var key in QQMiniAdapter.subNativeFiles){
							var tempArr=QQMiniAdapter.subNativeFiles[key];
				...
				if (tempurl.indexOf(QQMiniAdapter.window.qq.env.USER_DATA_PATH)==-1 &&(url.indexOf("http://")!=-1 || url.indexOf("https://")!=-1)&& !QQMiniAdapter.AutoCacheDownFile){
					...
					if(fileObj){
						...
						if(type !=/*laya.net.Loader.IMAGE*/"image" && ((url.indexOf("http://")==-1 && url.indexOf("https://")==-1)|| MiniFileMgr$6.isLocalNativeFile(url))){
							...
							MiniFileMgr$6.downFiles(url,encoding,new Handler(MiniLoader,MiniLoader.onReadNativeCallBack,[encoding,url,type,cache,group,ignoreCache,thisLoader]),url,cache);
						}
					}
				}
				}else {
				...
				MiniFileMgr$6.readFile(nativepath,fileObj.encoding,new Handler(MiniLoader,MiniLoader.onReadNativeCallBack,[encoding,url,type,cache,group,ignoreCache,thisLoader]),URL.formatURL(url));
			}
		}
	}
...
var MiniSound$6=(function(_super){
	...
	__class(MiniSound,'laya.qq.mini.MiniSound',_super,'MiniSound$6');
	...
	*/
	__proto.load=function(url){
		...
		if(QQMiniAdapter.autoCacheFile&&MiniFileMgr$6.getFileInfo(url)){
	...
	*/
	__proto.play=function(startTime,loops){
		...
		if(QQMiniAdapter.autoCacheFile&&MiniFileMgr$6.getFileInfo(this.url)){
			var fileNativeUrl;
...
var MiniSoundChannel$6=(function(_super){
	...
	__class(MiniSoundChannel,'laya.qq.mini.MiniSoundChannel',_super,'MiniSoundChannel$6');
